...
#BLOSUM DP AND TRACEBACK
def affine_local_alignment_blosum_and_traceback(seq1, seq2, blosum_matrix, g_open, g_ext):
  ...
  for i in range(m + 1):
    for j in range(n + 1):
      for matrix, tb_matrix in [(C, C_tb), (B, B_tb), (A, A_tb)]:
...
def affine_local_alignment_model_and_traceback(seq1, seq2, g_open = -1, g_ext = -0.25, model = "", tokenizer = ""):
  ...
  emb1 = get_embs_T5(seq1, Model, Model_tokenizer).cpu().numpy()
  emb2 = get_embs_T5(seq1, Model, Model_tokenizer).cpu().numpy()
...
def get_visualization(prot1, prot2 , score , Type = "" , Model = "" , Model_Tokenizer = ""):
  ...
  if Type == "ProtT5":
    ...
    p1_emb = get_embs_T5(no_dash[0], model, tokenizer)
    p2_emb = get_embs_T5(no_dash[0], model, tokenizer)
  ...
  for i in range(len(prot1)):
    ...
    if p1_revived[-1] == p2_revived[-1]:
      ...
      if Type in BLOSUMS_LIST:
        bl_no = Type[-2:]
        sim = bl.BLOSUM(int(bl_no))[p1_revived[-1]][p2_revived[-1]]

      elif Type in MODELS_LIST:
...
def run_individual_method_tests(ID, example_pairs, method, gap_open, gap_extension, gap_openT="", gap_extensionT="", shift=0, model="", tokenizer="", N=-1):
  ...
  for n in range(0,num_examples):
    ...
    file_name = f"{ID}_test{n+1}_{method}_{shift:1.5f}_{gap_open:1.5f}_{gap_extension:1.5f}.txt"
    with open(file_name, 'w') as file: #create the file, if one with the same file name already exists, we override
            ...
            if method.startswith("BLOSUM"):
                last_two_characters = method[-2:] #get the type BLOSUM matrix we should use
                S = bl.BLOSUM(int(last_two_characters))
                alignment, score = affine_local_alignment_blosum_and_traceback(protein1_seq, protein2_seq, S, gap_open, gap_extension)
            ...
            #the first method used an Embedding model
            else:
                ...
                file.write("\n  Gap Open Penalty: " + str(gap_open))
                file.write("\n  Gap Extension Penalty: " + str(gap_extension))
                file.write("\n  Gap Open Penalty T: " + str(gap_openT))
                file.write("\n  Gap Extension Penalty T: " + str(gap_extensionT))
                file.write("\n  Shift Score Down by: " + str(shift))
                file.write("\nScore: " + str(score))
    ...
    file.close()
  all_distances.to_csv(f"{ID}_{method}_{shift:.5f}_{gap_open:.5f}_{gap_extension:.5f}_distances.csv")
  shutil.move(f"{ID}_{method}_{shift:.5f}_{gap_open:.5f}_{gap_extension:.5f}_distances.csv", os.path.join(os.getcwd(), f"{ID}_tests/{ID}_distances"))
  return

def main():
  
  parms = pd.read_csv('parms.csv')
  model, tokenizer = ProtT5_initialize() #protT5

  examples = pd.read_csv("cd01068_examples.csv")
  
  #run the tests for each domain
  for i in range(0, examples.shape[0]):
    run_individual_method_tests("cd01068", f"{str(sys.argv[1])}_examples.csv", "ProtT5", parms.iloc[i][1], parms.iloc[i][2], model=model, tokenizer=tokenizer)
